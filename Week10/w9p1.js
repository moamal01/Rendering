...
async function main() {
  ...
  // Texture
  const texture = await load_texture(device, "../common/luxo_pxr_campus.hdr");

  // Buffers
  var drawingInfo = await readOBJFile("../objects/teapot.obj", 1.0, true);
  ...
  const bindGroup = device.createBindGroup({
    layout: pipeline.getBindGroupLayout(0),
      entries: [
        ...
        { binding: 10, resource: { buffer: lightIndicesBuffer } },
      ],
    }
  );
